...
###############################################################################
#
# User Definitions. In the following sections everything a user needs to
# define can be found
#
# There should also be a folder called files in the directory, where the
# software can save results to
#
###############################################################################

RAMOFMACHINE = 1    # the ram availible on your computer in Gbyte

# which calculation should be performed afterwards should be defined here.
# default is "0", "1" meaning, only direct connections between the two flexible
# ends, "2" only right angles between the two ends, "3" flexible ends taken as
# straigt parts, up to 3 angles

functionnumber = "0"

# A unique name under which checkpoints are created
UserDefinedProjectName = "lambdalysozyme"


UserChosenIgnoreRegions = None
#is a list of tuples of size 3, first entry being the first aminoacid to be
#ignored, second the last aminoacid, then the identifier of the subunit
# example : (1, 5, "A") #ignore aminoacid 1 to 5 in subunit A

IgnoreSubUnit = ("")
 # is a tupel with to be ignored subunits Exemple: ("A", "B")
...
# Choose the start and the end that should be circularized by the linker in
# aminoacids and choose the subunit.

ScarsAtStartseq = "GG"
ScarsAtEndseq = "RGKCWE"

# These are the sequences that should be before the rigid linker and after.
# The part after is due to circularization with inteins, RGKCWE is recommended.
# Before the linker there should be a pattern prohibiting the helix to proceed
# into the protein, "GG" is recomended.

pdbfile = 'proteine/xyz.pdb'
...
# UserdefinedWeighing follows the shape: 273,10 280-290,5 298,7,35.6  etc..
# spaces separate entries.
# If there are only two entries separated by "," this is used as one aminoacid
# with a certain weighting.
# "-" is used  for ranges of aminoacids, no substrate can be used and behind
# the weighting is added
# If there are three entries, the second is used for the diameter of the
# substrate and last for the weighting value)
...
if MissingAtStart > 0:
    if MissingAtStart >= 5:
        print "Your PDB File does not contain the beginning of the protein " +\
        "and there is missing that much, that even an estimation of the " +\
        "beginning would be really bad. Please find a better PDB file, " +\
        "or risk it"
    else:
        print "Your PDB File does not contain the beginning of the protein," +\
        " but there are less than 5 Aminoacids missing, so the position of " +\
        "the beginning will be estimated. If there is a better PDB file, " +\
        "upload it"

if MissingAtEnd > 0:
    if MissingAtEnd >= 5:
        print "Your PDB File does not contain the end of the protein and " +\
        "there is missing that much, that even an estimation of the " +\
        "beginning would be really bad. Please find a better PDB file," +\
        " or risk it"
    else:
...
if (functionnumber == "2") | (functionnumber == "0"):
    ...
    if keep.any():
        ...
        my_log( "firstpointstriangle = " + str( np.shape(firstpointstriangle)))
...
if (functionnumber == "3") | (functionnumber == "0"):
    ...
    if "Exit" not in catch:
        ...
        sys.exit(catch)
    my_log("aussortierennachpunkten gestartet")
    ...
    my_log("sort_out_by_prot started, 2nd Points")
    ...
    my_log("dritte Punkte am Erzeugen")
    if "Exit" not in catch:
        MakeSmall, teiler = catch
    else:
        sys.exit(catch)
...
if Exithere:
    ...
    my_log("not Exithere, started point weighing and translation")
