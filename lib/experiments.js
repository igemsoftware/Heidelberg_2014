...
experimentsVM = function (dataContainer) {
	...
	self.versions = data && _.map(data.experiment.v, function(version, index){
...
function makeSupplyPsource(supply) {
	var obj = {
		...
		getText: function (version) {
			var origin = 'obtained from ' + ko.unwrap(version.source) + ' on ' + UI._globalHelper('formatDate')(ko.unwrap(version.date));
