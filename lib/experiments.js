experimentsVM = function (dataContainer) {
	...
	self.protocol = data.experiment ? data.protocol.v[data.experiment.v[version].protocol.v] : data.protocol;
...
experimentVM = function (data, version, tryProtocolUpdate) {
	var self = this;
	...
	if (data.experiment) {
		...
		_.each(data.protocol.v[self.protocolVersion].params, function (param) {
			...
			if (param.multi) {
				...
				params[param.name] = self.getParamObs(param, sourceParam);
			}
		});

		var values = data.experiment ? ko.mapping.fromJS(version.values) : { };

		if (tryProtocolUpdate) {
