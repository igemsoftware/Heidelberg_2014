...
experimentVM.prototype.save = function (performer, changeType) {
	...
	if (!this.id) {
		...
		if (!_.isEqual(flat, this.data.v[this.data.v.length - 1])) Meteor.call('updateExperiment', this.id, flat, changeType);
		...
		_.each(newVersion.params, function (param) {
			if (param.push) {
				_.each(param, function (param) {
					if (param.paramValue.type == 'experiment') sourceExperiments.push(param.paramValue.experiment);
				})
			} else {
				if (param.type == 'experiment') sourceExperiments.push(param.experiment);
			}
		});
		Experiments.update(id, { $push: { v: newVersion }, $set: {
			finishDate: newVersion.finishDate,
			performer: newVersion.performer,
