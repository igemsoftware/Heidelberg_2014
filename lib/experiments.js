experimentsVM = function (dataContainer) {
	...
	var version = data.experiment && (typeof dataContainer.version() == 'undefined' ? data.experiment.v.length - 1 : dataContainer.version());
	self.oldVersion = data.experiment && typeof dataContainer.version() != 'undefined' && dataContainer.version() != data.experiment.v.length - 1;
	self.performer = ko.observable(data.experiment && data.experiment.v[version].performer);
	self.protocol = data.protocol;
	self.experiments = ko.observableArray([ new experimentVM(data, version) ]);
...
experimentVM = function (data, version) {
	var self = this;
	...
	if (data.experiment) _.each(data.protocol.params, function (param) {
		...
		if (param.multi) {
			self.params[param.name] = ko.observableArray(_.map(sourceParam, function (multiParam) {
				...
				obj.paramValue = self.getParamObs(param, multiParam.paramValue);
				return obj;
			}));
		} else {

			self.params[param.name] = self.getParamObs(param, sourceParam);
		}
	});
...
experimentVM.prototype.getParamObs = function (param, sourceParam) {
	...
	if (paramObs.peek() === undefined) {
		switch (sourceParam.type) {
			...
			case 'experiment':
				...
				var sourceProduct = _.find(sourceProtocol.products, function (pproduct) {
					return pproduct.name == sourceParam.product;
				});
				paramObs(makeMakeExperimentPsource([sourceProduct])(sourceExperiment));
				break;
		}
	...
	if (ko.unwrap(paramObs.peek().reference.v).length - 1 != sourceParam.v) {
	...
	paramObs.peek().href = paramObs.peek().getHref({ v: sourceParam.v });
...
function makeSupplyPsource(supply) {
	var obj = {
		...
		getText: function (version) {
			var origin = 'obtained from ' + ko.unwrap(version.source) + ' on ' + ko.unwrap(version.date);
	...
	return function (sourceExperiment) {
		return _.map(products, function (product, pid) {
			...
			var obj = {
				...
				editable: ko.observable(true),
				v: ko.unwrap(sourceExperiment.v).length - 1,
				currentVersionHref: null,
		...
		if (paramValue.push) {
			...
			switch (paramValue().type) {
				case 'supply':
					...
					sourceSupplies.push(paramValue().reference._id());
					break;
				case 'experiment':
					params[paramNameHash] = {
						type: 'experiment',
					...
					sourceExperiments.push(paramValue().reference._id());
					break;
			}
