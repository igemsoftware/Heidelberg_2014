...
ProtocolParam = function (data) {
	...
	var self = this;
	self.type = ko.observable(data ? pTypeMap[data.type._id] : self.types[0]);
		...
		if (type.v != obj.version) {
			if (!tryUpdate) {
				...
				_.each(obj.DBData.versionMappings, function (mapping) {
					...
					if (!inverseTypeMappings[mapping.source.from._id]) inverseTypeMappings[mapping.source.from._id] = { };
					inverseTypeMappings[mapping.source.from._id][mapping.source.name] = mapping.property;
				});
			}
		}
		return obj;
	...
	self.propertyBindings = ko.computed(function () {
		...
		_.each(self.types(), function (type) {
			...
			_.each(type.allProperties(), function (property) {
				...
				var mapping;
				if (data) {
					var mapping = tryUpdate && typeMappings[fromId] && typeMappings[fromId][property.name()];
							...
							if (data.propertyBindings[fromId] && data.propertyBindings[fromId][md5Name]) {
								...
								switch (binding.source.type) {
									case 'paramProperty':
										...
										var sourceMapping = inverseTypeMappings[sourceTypeId]
											&& inverseTypeMappings[sourceTypeId][binding.source.paramProperty.name];
...
ProtocolProductPropertyParamPropertySource.prototype.getValue = function (values, params) {
	return ko.unwrap(params()[this.param.name()]).getProperty(this.paramProperty);
};
...
ProtocolProductPropertyInputSource.prototype.getValue = function (values, params) {
	return values[this.stepIndex][this.inputIndex];
};
...
Meteor.methods({
	...
	updateProtocol: function (id, newVersion, oldVersionNo) {
		...
		cascadingUpdate(updateProtocol(id, newVersion), {
			type: 'cascade',
			dependency: {
				type: 'protocol',
				id: id,
