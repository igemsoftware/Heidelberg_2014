protocolVM = function (data) {
	...
	var version = self.data && (typeof data.version() == 'undefined' ? self.data.v.length - 1 : data.version());
	self.oldVersion = self.data && self.data.v && typeof data.version() != 'undefined' && data.version() != self.data.v.length - 1;
	self.name = ko.observable(self.oldVersion ? protocol && protocol.v[version].name : protocol && protocol.name);
	if(self.oldVersion) {
		self.params = ko.observableArray(protocol ? _.map(protocol.v[version].params, function (param) {
			return new protocolParamVM(param);
		}) : []);
		self.steps = ko.observableArray(protocol ? _.map(protocol.v[version].steps, function (step) {
			return new protocolStepVM(self, step);
		}) : []);
		self.products = ko.observableArray(protocol ? _.map(protocol.v[version].products, function (product) {
...
protocolVM.prototype.switchToVersion = function(id, version) {
	console.log(id, version);
	$("#versionsModal").modal('hide');
	Router.go('viewProtocol', { id: id }, { query: { v: version } });
