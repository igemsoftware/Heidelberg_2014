...
protocolVM = function (data, tryUpdate) {
	var self = this;
	self.editMode = data.editMode;
	self.data = ko.unwrap(data.protocol);
	...
	self.tryUpdate = tryUpdate;

	self.name = ko.observable(self.data && self.data.v[self.version].name);
	self.params = ko.observableArray(self.data ? _.map(self.data.v[self.version].params, function (param) {
		return new protocolParamVM(param);
	}) : []);
	self.steps = ko.observableArray(self.data ? _.map(self.data.v[self.version].steps, function (step) {
		return new protocolStepVM(self, step);
	}) : []);
	self.products = ko.observableArray(self.data ? _.map(self.data.v[self.version].products, function (product) {
		return new protocolProductVM(self, product, tryUpdate);
	}) : []);
...
protocolVM.prototype.flatten = function (changeType) {
	var failed = false;
	var flat = {
		name: this.name(),
		...
		products: _.map(this.products(), function (product) {
			var flatProduct = product.flatten();
			if (!flatProduct) failed = true;
			return flatProduct;
		}),
		mtime: new Date(),
	...
	flat.relevantTypes = _.flatten(_.map(flat.products, function (product) {
		return _.pluck(product.allTypes, '_id');
	}));
...
protocolVM.prototype.save = function (changeType) {
	var self = this;
	var flat = self.flatten(changeType);

	if (!flat) {
		// Should only ever be the case when running as privileged code, so no need for Meteor.methods()
		Protocols.update(self.id, { $set: { needsReview: true } });
	} else if (!self.id) {
		Meteor.call('insertProtocol', flat, function (error, result) {
			if (!error) {
				self.id = result;
				if (!changeType) Router.go('viewProtocol', { id: self.id });
			}
		});
		return;
		...
		if (xCount > 0) {
			...
			if (cannotUpdate && !changeType) {
				self.cannotCascadeModalVM({
					...
					xCount: xCount,
					save: function () {
						Protocols.update(self.id, { $push: { v: flat }, $set: flat, $unset: { needsReview: true } });
				...
				$('#mappingsModal').modal();
			} else if (cannotUpdate || needsMapping) {
				// Should only ever be the case when running as privileged code, so no need for Meteor.methods()
				Protocols.update(self.id, { $set: { needsReview: true } });
			} else {
				Router.call('updateProtocol', self.id, flat, self.data.v.length - 1);
			}
			return;
		} else {
			Protocols.update(self.id, { $push: { v: flat }, $set: flat, $unset: { needsReview: true } });
...
protocolParamVM.prototype.flatten = function () {
	return ko.toJS({
		type: _.extend({ v: this.type().v().length - 1 }, _.pick(this.type(), '_id', 'name')),
		name: this.name,
		multi: this.multi
	});
};
...
function protocolProductVM(protocolVM, product, tryUpdate) {
	...
	self.allTypes = ko.computed(function () {
		...
		_.each(self.types(), function (type) {
			map[type._id()] = type;
			var refType = product && !tryUpdate &&_.find(product.types, function (ptype) {
				return ptype._id == type._id();
			});
			_.each((refType ? type.v()[refType.v] : type).baseTypes(), function (baseType) {
	...
	self.propertyBindings = ko.computed(function () {
		...
		_.each(self.types(), function (type) {
			...
			var refType = product && !tryUpdate && !ko.unwrap(protocolVM.editMode) && _.find(product.types, function (ptype) {
				return ptype._id == type._id();
			});
			_.each((refType ? type.v()[refType.v] : type).allProperties(), function (property) {
				...
				if (map[property.from._id()] != 1) {
					...
					if (product) {
						var fromId, md5Name;
						var mapping = tryUpdate && type.versionMappings && _.find(type.versionMappings(), function (mapping) {
							return mapping.property.from._id() == property.from._id() && mapping.property.name() == property.name();
						});
						if (mapping) {
							fromId = (ko.unwrap(mapping.source.from) || mapping.property.from)._id();
							md5Name = CryptoJS.MD5(mapping.source.name()).toString();
						} else {
							fromId = property.from._id()
							md5Name = CryptoJS.MD5(property.name()).toString();
						}

						var source;
						if (product.propertyBindings[fromId] && product.propertyBindings[fromId][md5Name]) {
							source =_.find(obj.possibleSources(), function (psource) {
								// Meteor 0.8 ships with underscore 1.5.2, which does not yet implement _.matches, so we'll have to do it ourselves
								return _.every(product.propertyBindings[fromId][md5Name].source, function (value, key) {
...
protocolProductVM.prototype.flatten = function () {
	...
	if (!propertyBindings) return false;
	...
	_.each(propertyBindings, function (binding) {
		...
		propertyBindingsMap[binding.property.from._id()][CryptoJS.MD5(binding.property.name()).toString()] = {
			property: {
	...
	return {
		...
		types: ko.toJS(_.map(this.types(), function (type) {
			return _.extend({ v: type.v().length - 1 }, _.pick(type, '_id', 'name'));
		})),
		allTypes: ko.toJS(_.map(this.allTypes(), function (type) {
			return _.extend({ v: type.v().length - 1 },_.pick(type, '_id', 'name'));
		})),
		propertyBindings: ko.toJS(propertyBindingsMap)
	};
};

Meteor.methods({
	insertProtocol: function (id, flat) {
		if (!this.userId) return;
		flat.editor = this.userId;
		self.id = Protocols.insert(_.extend({
			v: [flat]
		}, flat));
	},
	updateProtocol: function (id, newVersion, oldVersionNo, paramMappings, inputMappings) {
		...
		if (paramMappings || inputMappings) {
			newVersion = _.extend({ versionMappings: { params: paramMappings, inputs: inputMappings } }, newVersion);
		}
