					...
					saveWithCascade: function () {
						...
						_.each(inputMappings, function (inputMapping) {
							if (!inputMapping.blank()) {
								...
								inputMappingsMap[inputMapping.stepIdx][inputMapping.inputIdx] = { step: inputMapping.source().stepIdx, input: inputMapping.souce().inputIdx };
							}
						});

						Meteor.call('updateProtocol', self.id, flat, self.data.v.length - 1, flatParamMappings, inputMappingsMap);
						if (Meteor.isClient) $('#mappingsModal').on('hidden.bs.modal', function () {
							Router.go('viewProtocol', { id: self.id });
						});
					}
				});
