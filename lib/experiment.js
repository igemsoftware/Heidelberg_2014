	...
	switch (data.type) {
		case 'supply':
			...
			if (paramSource === undefined || (!tryUpdate && paramSource.supply.version != data.v)) {
				...
				paramSource = new ExperimentParamSourceSupply(new Supply(Supplies.findOne(data.supply)), data.v);
		...
		case 'experiment':
			...
			if (paramSource === undefined || (!tryUpdate && paramSource.experiment.version != data.v)) {
				...
				var sourceExperiment = Experiments.findOne(data.experiment);
...
ExperimentParamMultiSource = function (experiment, param, paramSource, tryUpdate) {
	this.paramValue = param ? experiment.getExperimentParamSource(param, paramSource.paramValue, tryUpdate) : ko.observable({ editable: ko.observable(true) });
...
Meteor.methods({
	...
	updateExperiment: function (id, newVersion) {
		...
		cascadingUpdate(updateExperiment(id, newVersion), {
			type: 'cascade',
			dependency: {
				type: 'experiment',
				id: id,
